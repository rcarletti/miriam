...
func New(clientID string, secretFile string, scopes ...string) (*http.Client, error) {
	b, err := ioutil.ReadFile(secretFile) //read client's secret
	if err != nil {
		return nil, err
	}
	...
	return getClient(context.Background(), config, clientID), nil
}
